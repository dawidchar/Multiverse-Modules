...
while (blocks[blocks.length - 1].length < 8) {
    const pushChar = getValidChar((blocks[blocks.length - 1].reduce((acc, cur) => acc + cur, 0)))
    blocks[blocks.length - 1].push(pushChar)
}

function getValidChar(charCode, multiplier) {
    if (between(charCode, ...numberRange)) {
        return Math.floor(charCode)
    }
    if (between(charCode, ...letterRange)) {
        return Math.floor(charCode)
    }
    if (!multiplier) multiplier = 1
    const newCode = charCode < numberRange[0] ? charCode * multiplier : charCode / multiplier
    return getValidChar(newCode, multiplier + 0.01)
}

const getChar = (charCode) => !isNaN(charCode) ? String.fromCharCode(getValidChar(((charCode ^ 2) + 1))) : 1

const getStateTotal = (state) => state.reduce((acu, cur) => acu + cur, 0) % 376


function hashBlocks(blocks) {
    const state = [8, 24, 81, 90, 38, 56, 78, 97]
    blocks.forEach(block => {
        state[0] = (block.reduce((memo, b, index) => (index % 2) ? memo * (b) ? b : 2 : memo / (b) ? b : 2, 100) * block[0]) + getStateTotal(state)
        state[1] = (block.reduce((memo, b, index) => (index % 2) ? memo * (b) ? b * 3 : 2 : memo / (b) ? b * 3 : 2, state[0])) + getStateTotal(state)
        state[2] = (block.reduce((memo, b, index) => ((memo + b + index) * 2), block[block.length - 1])) + getStateTotal(state)
        state[3] = (block.reduce((memo, b) => memo + b, block[3])) + getStateTotal(state)
        state[4] = (block.reduce((memo, b, index) => (memo + b) * (index / 46), block[4])) + getStateTotal(state)
        state[5] = (block.reduce((memo, b, index) => (memo * index) + b + block[0], block[3])) + getStateTotal(state)
        state[6] = (block.reduce((memo, b, index) => (memo - index) + b, block[2]) ^ 4) + getStateTotal(state)
        state[7] = (block.reduce((memo, b, index) => memo + b * (index / 37), block[1])) + getStateTotal(state)
    })
    // console.log(state)
    return state.map(el => getChar(el)).join("")
}
